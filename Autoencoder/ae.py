...
def create_model():
    ...
    '''
    model = Sequential()
    model.add(Dense(720*480, 32*32, init='uniform', activation='tanh'))
    model.add(Dense(32*32, 720*480, init='uniform', activation='tanh'))
    return model
...
data = data[index]
#label = label[index]
(X_train,X_val) = (data[0:400],data[400:])
#(Y_train,Y_val) = (data[0:400],data[400:])
best_accuracy = 0.0
print(Ready)
for e in range(nb_epoch):
    ...
    for i in range(batch_num):
        loss,accuracy = model.train(X_train[i*batch_size:(i+1)*batch_size], X_train[i*batch_size:(i+1)*batch_size],accuracy=True)
        progbar.add(batch_size, values=[("train loss", loss),("train accuracy:", accuracy)] )

    #save the model of best val-accuracy
    print("Validation...")
    val_loss,val_accuracy = model.evaluate(X_val, X_val, batch_size=1,show_accuracy=True)
