function new = bilinearResize(pic, r, c)
  ...
  if numel(oldSize) == 2  %# Interpolate grayscale image
    new = interp2(pic, newX, newY(:), 'bilinear');
  else                    %# Interpolate RGB image
    new = zeros([newSize 3]);  %# Initialize new image
    new(:,:,1) = interp2(pic(:,:,1), newX, newY(:), 'bilinear');  %# Red plane
    new(:,:,2) = interp2(pic(:,:,2), newX, newY(:), 'bilinear');  %# Green plane
    new(:,:,3) = interp2(pic(:,:,3), newX, newY(:), 'bilinear');  %# Blue plane
  end
